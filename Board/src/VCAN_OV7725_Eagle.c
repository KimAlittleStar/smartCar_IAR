...
volatile uint8 IMG_Resouse = 0;
volatile uint8 IMG_NUM = 0;
...
void ov7725_eagle_port_init()
{
    ...
    dma_portx2buff_init(CAMERA_DMA_CH, (void *)&PTD_B0_IN, (void *)ov7725_eagle_img_buff, PCLK_PIN, DMA_BYTE1, CAMERA_DMA_NUM, DADDR_KEEPON);
...
//	port_init_NoALT(PTD7,PULLUP);
    ...
    port_init(PCLK_PIN, ALT1 | DMA_FALLING | PULLUP );         //PCLK

    port_init(VSYNC_PIN, ALT1 | IRQ_RISING | PULLUP | PF);     //场中断，上拉，上降沿触发中断，带滤波
	gpio_init(HERF_PIN,GPI,0);
	port_init_NoALT(HERF_PIN,PULLDOWN);
...
void ov7725_eagle_vsync(void)
{
    ...
    {
	  	while((HERF_IN)==0);			//新添加 等待行中断信号到齐 开始采集数据 
							//优化 第一时间恢复 DMA 相关设置 （效果不大 有效的 区间时间延时 在 500us之内都可用）
	  	DMA_DADDR(CAMERA_DMA_CH) = (uint32)(ov7725_eagle_img_buff+(600*IMG_NUM));    //恢复地址
        DMA_EN(CAMERA_DMA_CH);                  //使能通道CHn 硬件请求
        ov7725_eagle_img_flag = IMG_GATHER;                  //标记图像采集中
        disable_irq(PORTC_IRQn);
        PORTC_ISFR = 1 <<  PCLK_PIN;            //清空PCLK标志位
 ...
 *  @since      v5.0
 */
uint8* ov7725_eagle_get_img()
{
    uint8* imgdata;
    if(ov7725_eagle_img_flag == IMG_FINISH)
    {/*!< 如果标志位为结束 则意味着上一次图像接受完毕 */
        ov7725_eagle_img_flag = IMG_START;                   //开始采集图像
        PORTC_ISFR = ~0;                        //写1清中断标志位(必须的，不然回导致一开中断就马上触发中断)
        enable_irq(PORTC_IRQn);
        imgdata = (IMG_NUM)?(ov7725_eagle_img_buff):(ov7725_eagle_img_buff+600); 
    }
    else
    {/*!< 否则 等待 图像传输完毕 */
